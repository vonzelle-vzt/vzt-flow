...
| Minimum OS | **macOS 13.0** (`tauri.conf.json`'s `minimumSystemVersion`). ScreenCaptureKit is hard-linked (`LC_LOAD_DYLIB`, not weak) and its audio capture is 13.0+, so a lower floor would let an older Mac install the app and then dyld-abort at launch. | **macOS 13.3**, set per-target. Microsoft's official `onnxruntime-osx-x86_64-1.22.0` dylib has `LC_BUILD_VERSION` `minos 13.3`, and dyld refuses to load it below that. Because `minimumSystemVersion` isn't arch-aware in `tauri.conf.json`, the `macos-x64` release job overrides it via `--config` (which deep-merges per RFC 7396, so `signingIdentity`/`hardenedRuntime` survive). Both dmg guards assert the expected floor, so a silent regression to 12.0 fails the release. |
