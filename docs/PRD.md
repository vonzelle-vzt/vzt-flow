...
| Rolling transcription (transcribe-during-recording) | ✅ | `crates/flow-core/src/rolling.rs`. For long holds, silence-completed chunks are cut (reusing the chunker's `plan_cut` + seam-dedup) and transcribed on the shared engine **while recording continues**, so at release only the final <35s tail runs. Measured on a ~7min45s (465s) M5 clip: end-latency after release **25.15s → 0.53s** (batch vs. rolling), identical 7745-char transcript, 15 chunks transcribed during recording. Audio is streamed via an incremental resampler and dropped per-chunk so the capture buffer stays small (peak RSS remains ONNX-inference-bound, ~11GB, unchanged). A subdued live-preview line shows the raw tail during recording. Config `rolling_transcription` (default on); recordings <35s are unaffected (no chunks → single transcribe at release). Verify with hidden `flow rolling-test <file>`. |
| Accidental-press guard | ✅ | Right Option is macOS's special-character modifier (Option+e = ´ …). Holding it and pressing any other key is treated as typing, not push-to-talk: the tap (`crates/flow-core/src/hotkey.rs`) reports the keyDown-during-hold, the coordinator suppresses the hands-free toggle and **discards** any already-started recording. `ListenOnly` tap, so the typed character still reaches the app. |
| Paste verification | ✅ | `crates/flow-core/src/insert.rs`. ~150ms after the synthetic Cmd+V, the focused field's value is read via the Accessibility API (`kAXFocusedUIElement`→`kAXValue`); if readable and the transcript tail is missing, the paste is retried once, then (still missing) left on the clipboard with a "paste may have failed" overlay + notification. Unreadable value (most web/Electron/secure fields) → assume success (prior behavior). Bounded to <400ms. |
